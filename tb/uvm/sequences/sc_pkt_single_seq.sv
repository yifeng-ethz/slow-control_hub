class sc_pkt_single_seq extends uvm_sequence #(sc_pkt_seq_item);
  `uvm_object_utils(sc_pkt_single_seq)

  sc_type_e       sc_type;
  logic [23:0]    start_address;
  int unsigned    rw_length;

  function new(string name = "sc_pkt_single_seq");
    super.new(name);
    sc_type       = SC_READ;
    start_address = 24'h0;
    rw_length     = 1;
  ...
  task body();
    ...
    if (req_h.is_write()) begin
      for (int unsigned idx = 0; idx < rw_length; idx++) begin
        req_h.data_words_q.push_back(32'hA500_0000 + idx);
      end
    end
