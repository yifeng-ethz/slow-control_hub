...
-- Realization:
	...
	-- The issue now is how to translate the address code, which can be handled by the CAM IP.
	-- As the address map info is stored in sopc info 
	-- and can be export as header files. Such information must be loaded into CAM for searching before
	-- nCONFIG is done. So, initialization of the CAM is needed. The resource needed is not huge if 
	-- only the start of the address is used. 
...
architecture rtl of sc_hub is 
	...
	signal K285						: std_logic_vector(7 downto 0) := "10111100"; -- 8#BC#
	signal K284						: std_logic_vector(7 downto 0) := "10011100"; -- 8#9C#
	signal K237						: std_logic_vector(7 downto 0) := "11110111"; -- 8#F7#
